...
class WhatsAppBusinessAnalyticsService(BaseIntegrationService):
    """
    WhatsApp Business Analytics integration service.
    Provides read-only access to WhatsApp Business messaging analytics.
    """
    ...
    def connect(self, auth_code: str, redirect_uri: str) -> Dict[str, Any]:
        """
        Connect to WhatsApp Business using Facebook OAuth.
        """
        ...
        return {
            **token_data,
            'account_name': account_info.get('name', 'WhatsApp Business'),
            'account_id': account_info.get('id', '')
        }
    
    def refresh_access_token(self) -> str:
        """
        Refresh WhatsApp Business access token.
        """
        from campaigns.integrations.services.oauth import OAuthService
    ...
    def fetch_analytics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """
        Fetch WhatsApp Business messaging analytics.
        """
        access_token = self.integration.get_access_token()
        phone_number_id = self.integration.account_id
        
        if not phone_number_id:
            raise ValueError('Phone Number ID not configured')
        
        # Format dates for WhatsApp API
        ...
        params = {
            ...
            'fields': 'messages_sent,delivered,read,failed',
            'granularity': 'DAILY',
        ...
        response = requests.get(
            f'{self.api_base}/{phone_number_id}/insights',
            params=params
        )
            ...
            for metric_data in data['data']:
                ...
                for value_entry in values:
                    ...
                    if metric_name == 'messages_sent':
                        ...
                        analytics_by_date[date_str]['conversions'] = int(value)  # Track failures
    ...
    def validate_connection(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f'{self.api_base}/{phone_number_id}',
                params=params
            )
    ...
    def _fetch_account_info(self, access_token: str) -> Dict[str, Any]:
        """
        Fetch WhatsApp Business account information.
        """
        params = {'access_token': access_token}
        
        # Get WhatsApp business accounts
        response = requests.get(
            f'{self.api_base}/me?fields=owned_whatsapp_business_accounts',
            params=params
        )
        
        if response.status_code == 200:
            data = response.json()
            if data.get('owned_whatsapp_business_accounts', {}).get('data'):
                waba_id = data['owned_whatsapp_business_accounts']['data'][0]['id']
                response = requests.get(
                    f'{self.api_base}/{waba_id}',
                    params=params
                )
                if response.status_code == 200:
